...
export default function AnalyticsPage() {
  ...
  const getSelectedMonthInstallment = (): number | null => {
    if (!selectedMonth) return null;
    const found = months.find((m) => m.month === selectedMonth);
...
const handleSetHapto = async () => {
  ...
  if (isDashboardLoading && !hasDataLoaded) {
    return (
      <>
        {/* Desktop skeleton (visible md+) */}
        <div className="hidden md:block p-4 md:p-6 space-y-6">
        ...
        <div className="md:hidden p-4 space-y-4">
          {/* top bar skeleton (search + month) */}
          <div className="sticky top-0 z-30 bg-white px-4 py-2 flex items-center justify-between gap-3">
          ...
          <div className="grid grid-cols-2 gap-2 mt-3">
            {Array.from({ length: 6 }).map((_, index) => (
              <SkeletonMobileCard key={`init-mobile-skel-${index}`} />
            ))}
          </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2 flex-wrap">
                ...
                <Badge variant="outline" className="bg-blue-100 text-blue-800">
                  {getSelectedMonthInstallment() !== null
                    ? `Monthly: ₹${getSelectedMonthInstallment()}`
                    : "Monthly: Not Set"}
                </Badge>
            ...
            filteredMemberData.map((row, index) => {
              ...
              return (
                <div
                  key={row._id}
                  className="
                    bg-white border border-b-emerald-500 shadow-sm
                    rounded-md p-2
                    flex flex-col
                    min-h-[210px]
                  "
      ...
      "
        >
          ...
          <button
            ...
            className="flex flex-col items-center text-xs px-3"
            disabled={isMonthLoading || filteredMemberData.length === 0}
          >
